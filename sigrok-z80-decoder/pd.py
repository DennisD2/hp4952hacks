...
class Ann:
    ADDR, MEMRD, MEMWR, IORD, IOWR, INSTR, ROP, WOP, WARN, WAIT = range(10)
class Row:
    ADDRBUS, DATABUS, INSTRUCTIONS, OPERANDS, WARNINGS = range(5)
class Pin:
    D0, D7 = 0, 7
    M1, RD, WR, MREQ, IORQ, WAIT = range(8, 14)
    A0, A15 = 14, 28
class Cycle:
    NONE, MEMRD, MEMWR, IORD, IOWR, FETCH, INTACK, WAIT = range(8)
...
class Decoder(srd.Decoder):
    ...
    id       = 'z80dd'
    name     = 'Z80dd'
    longname = 'Zilog Z80 CPU DD'
    desc     = 'Zilog Z80 microprocessor disassembly. DD'
    license  = 'gplv3+'
    inputs   = ['logic']
    outputs  = []
    ...
    annotation_rows = (
        ...
        ('databus', 'Data bus', (Ann.MEMRD, Ann.MEMWR, Ann.IORD, Ann.IOWR, Ann.WAIT)),
    ...
    def decode(self):
        while True:
            ...
            if pins[Pin.MREQ] != 1: # default to asserted
                if pins[Pin.RD] == 0:
                    if pins[Pin.WAIT] == 0:
                        cycle = Cycle.WAIT if pins[Pin.M1] == 0 else Cycle.FETCH
                    else:
