...
; (00000h to 0ffffh), 32 kB RAM (08000h to 0ffffh) and a 16c550 UART.
;
...
; simple jump at the RST 008h-location.)
;
...
disk_trx_skip:   ld      bc, hl          ; Write new LBA3/2 into BC
                ...
                push    bc              ; Save LBA3/2
                ld      bc, 0200h        ; 512 byte per block
...
disk_trx_msg_1r: defb    "READ: ", cr, lf
                 defb         "    MEMORY START=", eos
disk_trx_msg_1w: defb    "WRITE: ", cr, lf
                 defb         "    MEMORY START=", eos
...
;******************************************************************************
;
ide_data_low:    equ     ide_base + 00h
ide_data_high:   equ     ide_base + 08h
ide_error_code:  equ     ide_base + 01h
;
...
;
ide_secnum:      equ     ide_base + 02h
;
;       Typically set to 1 sector to be transf.
;
ide_lba0:        equ     ide_base + 03h
ide_lba1:        equ     ide_base + 04h
ide_lba2:        equ     ide_base + 05h
ide_lba3:        equ     ide_base + 06h
;
...
;               7    : Always 1
;
ide_status_cmd:  equ     ide_base + 07h
;
;       Useful commands (when written):
;
;               020h: Read sectors with retry
;               030h: Write sectors with retry
;               0ECh: Identify drive
;
...
;
ide_retries:     equ     0ffh                     ; Number of retries for polls
;
;
...
ide_get_id:      push    af
                ...
                ld      b, 00h                   ; We will read 256 words
...
ide_ws:          push    bc
                ...
                call    ide_bfr_ready           ; Is the buffer ready to read?
                jr      c, ide_ws_err           ; No
                ld      b, 00h                   ; We will write 256 word
...
ide_set_lba:     push    af
                ...
                and     00fh                     ; Only bits 0 - 3 are LBA3
                or      0e0h                     ; Select LBA and master drive
                out     (ide_lba3), a
                pop     af
                ret
;
...
ide_ready_loop:  in      a, (ide_status_cmd)     ; Read drive status
                and     a, 0c0h                  ; Only bits 7 and 6 are needed
                xor     040h                     ; Invert the ready flag
...
dump_fcb_2:      defb    cr, lf, tab
                 defb     "File name      : ", eos
dump_fcb_3:      defb    cr, lf, tab
                 defb     "File size      : ", eos
dump_fcb_4:      defb    cr, lf, tab
                 defb     "1st cluster    : ", eos
dump_fcb_5:      defb    cr, lf, tab
                 defb     "File type      : ", eos
dump_fcb_6:      defb    cr, lf, tab
                 defb     "File pointer   : ", eos
dump_fcb_7:      defb    cr, lf, tab
                 defb     "Current cluster: ", eos
dump_fcb_8:      defb    cr, lf, tab
                 defb     "Current sector : ", eos
;
...
str2filename:    push    af
                ...
                ld      b, 0bh                   ; Fill 11 bytes with spaces
...
; be found, 00000h will be returned.
...
fopen:   push    af
                ...
                ld      bc, 00200h               ; address - this is the bfr siz.
...
dirlist_prtdir:  call    puts
                ld      h, (ix + 01ch + 3)       ; Get and print file size
                ld      l, (ix + 01ch + 2)
                call    print_word
                ld      h, (ix + 01ch + 1)
                ld      l, (ix + 01ch)
...
; Get and print start sector
                ld      a, tab
                call    putc
                ld      h, (ix + 01ah + 1)       ; Get cluster number
                ld      l, (ix + 01ah)
...
fatmount:       push    af
                ...
                ld      ix, buffer + 01feh       ; Check for 055AAh as MBR trailer
                ld      a, 055h
                cp      (ix)
                jp      nz, fatmount_e2
                ld      a, 0aah
                ...
                cp      (ix + 0ch)
                jp      nz, fatmount_e5
                ld      a, (buffer + 0dh)        ; Get cluster size
                ld      (clusiz), a
                ld      bc, (buffer + 0eh)       ; Get reserved sector number
                ld      (ressec), bc
                ld      bc, (buffer + 016h)      ; Get FAT size in sectors
                ld      (fatsec), bc
                ld      bc, (buffer + 011h)      ; Get length of root directory
                ld      (rootlen), bc
...
fatmount_5:      defb    "FATAL(FATMOUNT): Sector size not equal 512 bytes!"
                defb    cr, lf, eos
fatmount_s1:     defb    tab
                defb      "FATNAME:", tab, eos
fatmount_s2:     defb    cr, lf, tab
                defb      "CLUSIZ:", tab, eos
fatmount_s3:     defb    cr, lf, tab
                 defb     "RESSEC:", tab, eos
fatmount_s4:     defb    cr, lf, tab
                 defb     "FATSEC:", tab, eos
fatmount_s5:     defb    cr, lf, tab
                 defb     "ROOTLEN:", tab, eos
fatmount_s6:     defb    cr, lf, tab
                 defb      "PSIZ:", tab, tab, eos
fatmount_s7:     defb    cr, lf, tab
                 defb      "PSTART:", tab, eos
fatmount_s8:     defb    cr, lf, tab
                 defb      "FAT1START:", tab, eos
fatmount_s9:     defb    cr, lf, tab
                 defb      "ROOTSTART:", tab, eos
fatmount_sa:     defb    cr, lf, tab
                 defb     "DATASTART:", tab, eos
;
